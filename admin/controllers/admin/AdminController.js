...
exports.login = function (req, res) {
    ...
    if (req.method == "POST") { 
        ...
        if(errors){	  
            ...
            var sql="SELECT id, first_name, last_name, role_id FROM `users` WHERE `email`='"+email+"' and password = '"+password+"'";                           
            new Promise((resolve,reject)=>{
                connectPool.query(sql, (err, resp) => {
                  
                    // console.log("Response: "+JSON.stringify(resp.user));
                    // console.log("userData: "+resp[0].user);
                    // console.log("Error : "+err);
                    if (err) {
                        console.log("Error : "+err);
                        data.email = input.email;
                        data.password = input.password;
                        // errorData.email = resp.message;
                        // reject(err)
                    } else {

                        req.session.LoginUser = resp[0];
                        console.log("Loggedin Users : "+req.session.LoginUser);
