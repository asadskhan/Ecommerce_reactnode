...
{	 
	...
	try {
		var sql='select * from users where id != 0 AND is_active = 1 AND role_id = 1';   
...
async function getAllData() {	  
	try {   
			var sql='select * from users where id != 0 ORDER BY id DESC';
			 
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						console.log('MiddleWear Catch Error: '+err)
						reject(err)
					} else { 
						console.log('MiddleWear Success : '+result)
						resolve(result)
					}
				})
			})
