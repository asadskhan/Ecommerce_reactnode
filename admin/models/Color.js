...
async function checkRecordDuplication(name,id=null) {	  
	try { 
		if(name){
			if(id){
				var sql='select * from color WHERE `id` != '+ id + ' AND `title` = "'+ name + '" ';
			}else{
				var sql='select * from `color` WHERE `title` = "'+ name + '" ';
			} 
			console.log(sql);
...
async function deleteRecord(id) {	  
	try { 
		if(id){ 
			var sql='delete  from color where id = '+ id;   
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function getAllData() {	  
	try {   
			var sql='select * from color where is_deleted = 0';    
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function updateData(data) {	  
	...
	try { 
		if(data){   
			var sql = "UPDATE color set ? WHERE id = ?";   
			return new Promise((resolve,reject)=>{
...
module.exports={
	...
	{	
        var Product = sequelize.define('color', {
              id: { 
                type: type.INTEGER,
                primaryKey: true,
                autoIncrement: true
